...
async function makeComposite() {
	if (cam && screen) {
		canvasCtx.save();
		canvasElement.setAttribute("width", `${screen.videoWidth*1.252}px`);
		...
		canvasCtx.drawImage(
			cam,
			screen.videoWidth,
			Math.floor(screen.videoHeight - screen.videoHeight / 1),
...
async function mergeStreamsFn() {
	document.getElementById("mergeStreams").style.display = "block";
	await makeComposite();
